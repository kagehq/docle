...
<template>
  ...
  <Transition
    ...
    <div
      v-if="isOpen"
      class="fixed right-0 top-0 h-full w-full md:w-3/4 lg:w-2/3 xl:w-1/2 bg-black border-l border-gray-500/20 z-50 flex flex-col">

      <!-- Panel Header -->
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-500/20 flex-shrink-0">
        <div>
          <h2 class="text-xl font-bold text-white">Integration Snippets</h2>
          <p class="text-sm text-gray-400 mt-1">Copy-paste examples to integrate Docle</p>
        </div>
        <button
          @click="emit('close')"
          class="w-8 h-8 rounded-lg bg-gray-500/10 hover:bg-gray-500/20 text-gray-400 hover:text-white border border-gray-500/20 transition-all flex items-center justify-center">
        ...
        <div class="bg-gray-500/10 rounded-lg border border-gray-500/10 overflow-hidden">
          <div class="flex items-center justify-between px-4 py-3 border-b border-gray-500/20">
            <div class="flex items-center gap-2 text-sm text-gray-400">
              ...
              <span class="uppercase font-medium">{{ activeTab }}</span>
            </div>
            <button
